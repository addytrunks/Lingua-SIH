...
class LandmarkRenderer:
    def __init__(self, pose_connections_to_skip=None, hand_color=(0, 0, 255), pose_color=(255, 0, 0)):
        ...
        self.CONNECTIONS_NOT_NEEDED = [
            20, 18, 22, 21, 19, 17, 26, 25, 28, 27, 32, 29, 30, 31]
        ...
        if pose_landmarks:
            ...
            class PoseLandmarkObject:
                def __init__(self, landmarks):
                    self.landmark = [type('Landmark', (), {
                                          'x': lm['x'], 'y': lm['y'], 'z': lm.get('z', 0)}) for lm in landmarks]
            ...
            for connection in self.mp_pose.POSE_CONNECTIONS:
                ...
                start_x = int(
                    pose_landmark_obj.landmark[connection[0]].x * canvas.shape[1])
                start_y = int(
                    pose_landmark_obj.landmark[connection[0]].y * canvas.shape[0])
                end_x = int(
                    pose_landmark_obj.landmark[connection[1]].x * canvas.shape[1])
                end_y = int(
                    pose_landmark_obj.landmark[connection[1]].y * canvas.shape[0])

                # Draw connection line
                cv2.line(canvas, (start_x, start_y),
                         (end_x, end_y), self.pose_color, 2)
        ...
        if hand_landmarks:
            ...
            for hand_landmark_dict in hand_landmarks:
                ...
                for lm in hand_landmark_obj.landmark:
                    cx = int(lm.x * w)
                    cy = int(lm.y * h)
                    cv2.circle(canvas, (cx, cy), 2,
                               self.hand_color, cv2.FILLED)

                # Draw hand connections
                for connection in self.mp_hands.HAND_CONNECTIONS:
                    start_x = int(
                        hand_landmark_obj.landmark[connection[0]].x * w)
                    start_y = int(
                        hand_landmark_obj.landmark[connection[0]].y * h)
                    end_x = int(
                        hand_landmark_obj.landmark[connection[1]].x * w)
                    end_y = int(
                        hand_landmark_obj.landmark[connection[1]].y * h)

                    # Draw connection line
                    cv2.line(canvas, (start_x, start_y),
                             (end_x, end_y), self.hand_color, 1)
...
def render_sentence(words, renderer=None, coordinates_file="coordinates/final_coordinates.csv"):
    ...
    for word in words:
        ...
        if word_data.empty:
            word_data = df[df['word'] == word.upper()]
            if word_data.empty:
        ...
        for _, row in word_data.iterrows():
            ...
            # Prepare landmarks
            pose_landmarks = ast.literal_eval(
                row['pose']) if row['pose'] != '[]' else None
            hand_landmarks = ast.literal_eval(
                row['hands']) if row['hands'] != '[]' else None
            ...
            # Render landmarks
            canvas = renderer.render_landmarks(
                canvas, pose_landmarks, hand_landmarks)

            cv2.putText(canvas, word, position, font,
            ...
            yield img
            # socketio.emit('isl feed', "data:image/jpeg;base64,"+img, to=sid)
