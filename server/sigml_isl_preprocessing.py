...
def load_sigml_data(path="./data/sigmlfiles.json"):
    with open(path) as json_file:
        data = json.loads(json_file.read())
        return [i["name"] for i in data]


class SigmlISLPreprocessor:
    def __init__(self):
        self.multi_word_list = load_sigml_data()
        self.multi_word_list_names = [i["name"] for i in self.multi_word_list]
        self.list = None
        self.number_words = {
    ...
    def preprocess(self, sentence):
      ...
      """)
        ...
        final_keys = []
        for key in keys:
            if key in self.multi_word_list_names:
...
if __name__ == "__main__":
    load_dotenv()
    preprocessor = SigmlISLPreprocessor()
